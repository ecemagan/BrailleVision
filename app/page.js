...
function NemethCell({ symbol, dots }) {
  return (
    <article className="glass-card rounded-xl p-2.5">
...
export default function HomePage() {
  ...
  return (
    <main className="page-shell flex items-center justify-center">
      <section className="mx-auto grid w-full max-w-6xl gap-8 lg:grid-cols-[1.35fr_0.65fr] lg:items-stretch">
        <section className="surface-card hero-wash rounded-[32px] p-8 md:p-12">
          <p className="mb-6 inline-flex rounded-full border border-violet-200 bg-violet-50 px-4 py-2 text-sm font-semibold text-violet-700">
            {t("landing.badge")}
          </p>
          ...
          <nav aria-label="Landing actions" className="mt-8 flex flex-wrap gap-3">
            <Link
              href="/register"
          ...
          <section className="glass-card mt-7 rounded-2xl bg-white/75 p-5 backdrop-blur-md">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{t("landing.preview.title")}</p>
            <section className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_1.35fr] md:items-start">
              <section className="rounded-xl border border-purple-200/70 bg-white p-4">
              ...
              <section className="rounded-xl border border-purple-200/70 bg-white/90 p-4">
