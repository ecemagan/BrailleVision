...
function normalizeStructuredMathText(text) {
  return String(text || "")
    .replace(/\blim\s*x\s*S\s*c\b/giu, "lim x→c")
    .replace(/\blimxSc\b/giu, "lim x→c")
    .replace(/\blim\s*x\s*S\s*-\s*([0-9]+)/giu, "lim x→-$1")
    .replace(/\blimxS-([0-9]+)/giu, "lim x→-$1")
    .replace(/\bx\s*S\s*c\b/giu, "x→c")
    .replace(/\bxSc\b/giu, "x→c")
    .replace(/\bx\s*S\s*-\s*([0-9]+)/giu, "x→-$1")
    .replace(/\bxS-([0-9]+)/giu, "x→-$1")
    .replace(/[ \t]+/g, " ")
    .trim();
}

function normalizeLimitExpressionOrder(text) {
  let value = normalizeStructuredMathText(text);

  value = value.replace(/\blim\s+(?!x→)(.+?)\s+(x→(?:c|-?\d+))\b/giu, "lim $2 $1");

  return value.replace(/\s+/g, " ").trim();
}

export function normalizeRootRadicands(text) {
  let value = String(text || "");

  value = value.replace(
    /√\s*([0-9a-zA-Z^]+(?:\s*[*/·]\s*[0-9a-zA-Z^]+)*\s*[+-]\s*[0-9a-zA-Z^]+)/gu,
    "√($1)",
  );
...
export function normalizeNthRoots(text) {
  let value = String(text || "");

  value = value.replace(/√\s*n\s+([A-Za-z]\(x\))/gu, "ⁿ√$1");
  value = value.replace(/√\s*n\s+([A-Z])\b/gu, "ⁿ√$1");
  value = value.replace(/√n\s+([A-Za-z]\(x\))/gu, "ⁿ√$1");
  value = value.replace(/√n([A-Z])\b/gu, "ⁿ√$1");

  return value.replace(/\s+/g, " ").trim();
}

function normalizeStructuredMathBlockText(text) {
  return normalizeNthRoots(normalizeRootRadicands(normalizeLimitExpressionOrder(text)));
}

function normalizeExampleItemMath(text) {
  return normalizeStructuredMathBlockText(text);
}

function syncBlockTextFields(block, normalizedText, prefix = "") {
  const baseText = String(normalizedText || "").trim();
  const content = prefix ? `${prefix} ${baseText}`.trim() : baseText;

  block.originalContent = content;
  block.normalizedContent = content;
  block.text = baseText;
  block.content = content;
  block.originalText = content;

  return block;
}

function findInlineTheoremNoteStart(text) {
  const value = String(text || "");
  const match = /\((If|When|Assume|Provided)\b/iu.exec(value);
...
function parseExampleStart(text) {
  const value = String(text || "").trim();
  if (!value) {
    return null;
  }

  const match = /^(?:(Worked)\s+)?(EXAMPLE|Example)\s+(\d+(?:\.\d+)?)(?:\s*[:.\-]?\s*)?(.*)$/u.exec(value);
  if (!match) {
    return null;
  }

  const isWorked = Boolean(match[1]);
  const number = match[3];
  const prompt = match[4].trim();

  return {
    number,
    title: `${isWorked ? "WORKED EXAMPLE" : "EXAMPLE"} ${number}`,
    prompt,
  };
}

function parseExampleItemStart(text) {
  const value = String(text || "").trim();
  const match = /^\(([a-z])\)\s*(.*)$/iu.exec(value);
  if (!match) {
    return null;
  }
...
function findInlineExampleItem(text) {
  const value = String(text || "");
...
function splitExampleItemsInLine(text) {
  const value = String(text || "");
  const regex = /(\([a-z]\))/giu;
  const matches = [...value.matchAll(regex)];

  if (!matches.length) {
    return { before: value.trim(), items: [] };
  }

  const before = value.slice(0, matches[0].index).trim();
  const items = matches.map((match, idx) => {
    const start = match.index;
    const end = idx + 1 < matches.length ? matches[idx + 1].index : value.length;
    const chunk = value.slice(start, end).trim();
    const label = match[1].toLowerCase();
    const body = chunk.slice(label.length).trim();
...
function looksLikeTheoremNoteStart(text) {
  return /^\((?:If|Suppose|Let|Assume|For|When|Provided)\b/u.test(String(text || "").trim());
}
...
function isStrongExampleBoundary(text) {
  const value = String(text || "").trim();
  if (!value) {
    return false;
  }
  ...
  const keywordType = detectKeywordBlockType(value);
  if (keywordType && keywordType !== PAGE_BLOCK_TYPES.EXAMPLE) {
    return true;
  }
  ...
  if (/^\d+(?:\.\d+)+\s+\S/u.test(value)) {
    return true;
  }

  return looksLikeHeadingLine(value) && !parseExampleStart(value);
}
...
function splitEntryAroundInlineTheoremNote(entries, index) {
  const entry = entries[index];
  const value = String(entry?.text || "").trim();
  const inlineNote = findInlineTheoremNoteStart(value);

  if (!inlineNote || inlineNote.index <= 0) {
    return false;
  }

  const beforeNote = value.slice(0, inlineNote.index).trim();
  const notePart = value.slice(inlineNote.index).trim();

  if (!beforeNote || !notePart) {
    return false;
  }
...
function splitEntryAroundInlineExampleItem(entries, index) {
  const entry = entries[index];
  const value = String(entry?.text || "").trim();
  const inlineItem = findInlineExampleItem(value);

  if (!inlineItem || inlineItem.index <= 0) {
    return false;
  }

  const beforeItem = value.slice(0, inlineItem.index).trim();
  const itemPart = value.slice(inlineItem.index).trim();

  if (!beforeItem || !itemPart) {
    return false;
  }
...
function buildTheoremChildren(entries, { pageNumber = 1, parentId } = {}) {
  ...
  while (index < safeEntries.length) {
    ...
    if (splitEntryAroundInlineTheoremNote(safeEntries, index)) {
      continue;
    }
    ...
    if (looksLikeTheoremNoteStart(value)) {
      ...
      while (cursor < safeEntries.length) {
        ...
        if (splitEntryAroundInlineTheoremNote(safeEntries, cursor)) {
          continue;
        }
      ...
      children.push(
        syncBlockTextFields(
          ...
          noteEntries.reduce(
            (combined, noteEntry) => appendText(combined, normalizeStructuredMathText(noteEntry?.text)),
            "",
          ),
        ),
      );
    ...
    if (looksLikeRuleItemStart(value)) {
      ...
      while (cursor < safeEntries.length) {
        ...
        let ruleBody = normalizeStructuredMathBlockText(parsedRule.remainder);
        cursor += 1;
        ...
        while (cursor < safeEntries.length) {
          ...
          ruleBody = normalizeStructuredMathBlockText(appendText(ruleBody, nextText));
          cursor += 1;
        }

        ruleBody = normalizeStructuredMathBlockText(ruleBody);
        const ruleDisplayText = appendText(
          `${parsedRule.number}. ${parsedRule.title}${ruleBody ? ":" : ""}`,
          ruleBody,
        );
        const ruleItem = createTheoremChildBlock({
        ...
        syncBlockTextFields(ruleItem, ruleBody, `${parsedRule.number}. ${parsedRule.title}${ruleBody ? ":" : ""}`);
    ...
    if (isMathLikeLine(value) || shouldTreatTheoremLineAsMath(value)) {
      ...
      const equationText = mathEntries.reduce(
        (combined, mathEntry) => appendText(combined, normalizeStructuredMathBlockText(mathEntry?.text)),
        "",
      );
      ...
      syncBlockTextFields(child, equationText);
...
function buildExampleChildren(entries, { pageNumber = 1, parentId } = {}) {
  if (!Array.isArray(entries) || !entries.length || !parentId) {
    return { children: undefined, number: "", title: "", prompt: "" };
  }
  ...
  safeEntries.forEach((entry) => {
    const { before, items } = splitExampleItemsInLine(entry?.text);

    if (!items.length) {
      exampleEntries.push({ ...entry, text: String(entry?.text || "").trim() });
      return;
    }

    if (before) {
      exampleEntries.push({ ...entry, text: before });
    }

    items.forEach((item) => {
      exampleEntries.push({ ...entry, text: appendText(item.label, item.text) });
    });
  });
  ...
  let exampleTitle = "";
  let examplePrompt = "";
  let promptChild = null;

  function upsertPrompt(text, sourceEntry) {
    const nextPrompt = appendText(examplePrompt, text);
    examplePrompt = nextPrompt;

    if (!promptChild) {
      promptChild = createTheoremChildBlock({
        parentId,
        pageNumber,
        order: childOrder,
        type: "example_prompt",
        entries: [{ ...sourceEntry, text: nextPrompt }],
        confidence: 0.68,
        extra: {
          text: nextPrompt,
          content: nextPrompt,
          originalText: nextPrompt,
        },
      });
      children.push(promptChild);
      childOrder += 1;
      return;
    }

    promptChild.originalContent = nextPrompt;
    promptChild.normalizedContent = nextPrompt;
    promptChild.content = nextPrompt;
    promptChild.text = nextPrompt;
    promptChild.originalText = nextPrompt;
  }

  const heading = parseExampleStart(exampleEntries[0]?.text);
  if (heading) {
    exampleNumber = heading.number;
    exampleTitle = heading.title;
    if (heading.prompt) {
      const { before, items } = splitExampleItemsInLine(heading.prompt);
      if (before) {
        upsertPrompt(before, exampleEntries[0]);
      }

      if (items.length) {
        const injectedItems = items.map((item) => ({ ...exampleEntries[0], text: appendText(item.label, item.text) }));
        exampleEntries.splice(1, 0, ...injectedItems);
      }
    }
    index = 1;
  }

  while (index < exampleEntries.length) {
    ...
    const itemStart = parseExampleItemStart(value);
    if (itemStart) {
      let itemText = normalizeExampleItemMath(itemStart.remainder);
      let cursor = index + 1;

      while (cursor < exampleEntries.length) {
        const next = exampleEntries[cursor];
        const nextText = String(next?.text || "").trim();
        if (!nextText || parseExampleItemStart(nextText) || isStrongExampleBoundary(nextText)) {
          break;
        }

        if (!isEquationContinuationLine(nextText) && !isMathLikeLine(nextText) && !looksLikeEquationStep(nextText)) {
          break;
        }

        itemText = normalizeExampleItemMath(appendText(itemText, nextText));
        cursor += 1;
      }

      itemText = normalizeExampleItemMath(itemText);
      const itemBlock = createTheoremChildBlock({
        parentId,
        pageNumber,
        order: childOrder,
        type: "example_item",
        entries: [{ ...entry, text: appendText(itemStart.label, itemText) }],
        confidence: 0.72,
        extra: {
          label: itemStart.label,
        },
      });
      syncBlockTextFields(itemBlock, itemText, itemStart.label);
    ...
    while (cursor < exampleEntries.length) {
      ...
      if (children.some((child) => child.type === "example_item") && (isMathLikeLine(nextText) || looksLikeEquationStep(nextText))) {
        break;
      }

      promptEntries.push(next);
      promptText = appendText(promptText, nextText);
      cursor += 1;
    }

    if (children.some((child) => child.type === "example_item")) {
      children.push(
        createTheoremChildBlock({
          parentId,
          pageNumber,
          ...
          entries: [{ ...entry, text: promptText }],
          confidence: 0.66,
        }),
      );
    } else {
      upsertPrompt(promptText, entry);
      index = cursor;
      continue;
    }
    childOrder += 1;
...
function shouldKeepExampleBlockRunning(current, next, gap, typicalHeight, pageWidth) {
  ...
  if (isStrongExampleBoundary(next.text)) {
    return false;
  }
...
function buildBlocksFromRegion(region, { pageNumber = 1, pageWidth = 0 } = {}) {
  ...
  function flush() {
    ...
    if (type === PAGE_BLOCK_TYPES.EXAMPLE) {
      const exampleData = buildExampleChildren(current, { pageNumber, parentId: id });
      block.children = exampleData.children;
      block.number = exampleData.number || "";
      block.title = exampleData.title || contentLines[0] || "";
      block.prompt = exampleData.prompt || "";
      block.content = joined;
      block.text = joined;
      block.originalText = joined;
    }
  ...
  if (!isLayout) {
    ...
    function flush() {
      ...
      if (type === PAGE_BLOCK_TYPES.EXAMPLE) {
        ...
        block.children = exampleData.children;
        block.number = exampleData.number || "";
        block.title = exampleData.title || joined;
        block.prompt = exampleData.prompt || "";
        block.content = joined;
        block.text = joined;
        block.originalText = joined;
    ...
    for (const entry of safeLines) {
      ...
      if (shouldKeepTheoremBlockRunning(current, entry, 0, 12)) {
        current.push(entry);
        continue;
      }

      if (shouldKeepExampleBlockRunning(current, entry, 0, 12, 0)) {
        current.push(entry);
        continue;
      }
