...
function tokenizeMath(sourceText) {
  ...
  while (!isAtEnd()) {
    ...
    // Unknown character — skip silently (OCR artifacts, diacritics, etc.)
    advance();
    continue;
  }
...
/**
 * convertToBraille — paragraph-aware Braille converter.
 *
 * Guarantees that the output has the EXACT SAME paragraph structure
 * (blank-line boundaries) as the input.  This is critical for the
 * AlignedBrailleComparison UI: both sides must have identical paragraph
 * counts so that block 01 aligns with block 01, block 02 with 02, etc.
 *
 * Algorithm:
 *   1. Split input on blank-line boundaries → paragraphs.
 *   2. For every paragraph, split on single newlines → lines.
 *   3. Translate each line (math or plain text) — preserving the line itself.
...
export function convertToBraille(text) {
  ...
  const translatedParagraphs = paragraphs.map((para) => {
    // Each paragraph may have multiple internal lines (e.g. long wraps)
    const lines = para.split("\n");
    const translatedLines = lines.map((line) => convertLine(line));
